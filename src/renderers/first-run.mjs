...
function translateCheckLabel(label, language) {
  return {
    ko: {
      ...
      "AIGate enforcement": "AIGate 강제 연결",
      ...
      "hook activation automation": "hook 활성화 자동화",
      "local pre-push hook": "로컬 pre-push hook",
      "npm test script": "npm test script",
      "package.json": "package.json",
      "pre-push hook": "pre-push hook",
      "project foundation score": "프로젝트 기반 점수",
      "repository pre-push hook file": "저장소 pre-push hook 파일",
      "test script": "테스트 스크립트"
    },
    ja: {
      "AIGate config": "AIGate 設定",
      "AIGate enforcement": "AIGate 強制接続",
      "AIGate generated files version": "AIGate 生成ファイルバージョン",
      "CI workflow": "CI workflow",
      "Git repository": "Git リポジトリ",
      "Node.js runtime": "Node.js runtime",
      "git-ready gate": "git-ready ゲート",
      "hook activation automation": "hook 有効化自動化",
      "local pre-push hook": "ローカル pre-push hook",
      "npm test script": "npm test script",
      "package.json": "package.json",
      "pre-push hook": "pre-push hook",
      "project foundation score": "プロジェクト基盤スコア",
      "repository pre-push hook file": "リポジトリ pre-push hook ファイル",
      "test script": "テストスクリプト"
    },
    zh: {
      "AIGate config": "AIGate 配置",
      "AIGate enforcement": "AIGate 强制连接",
      "AIGate generated files version": "AIGate 生成文件版本",
      "CI workflow": "CI workflow",
      "Git repository": "Git 仓库",
      "Node.js runtime": "Node.js runtime",
      "git-ready gate": "git-ready 关卡",
      "hook activation automation": "hook 激活自动化",
      "local pre-push hook": "本地 pre-push hook",
      "npm test script": "npm test script",
      "package.json": "package.json",
      "pre-push hook": "pre-push hook",
      "project foundation score": "项目基础分",
      "repository pre-push hook file": "仓库 pre-push hook 文件",
      "test script": "测试脚本"
    }
...
function translateNextStep(step, language) {
  return {
    ko: {
      ...
      "Install the pre-push hook or add aigate git-ready to CI required checks.": "pre-push hook을 설치하거나 CI 필수 검사에 aigate git-ready를 추가하세요.",
      "Make aigate git-ready a required CI check or install the local pre-push hook.": "aigate git-ready를 CI 필수 검사로 만들거나 로컬 pre-push hook을 설치하세요.",
      "Make aigate git-ready a verified required CI check.": "aigate git-ready를 검증된 CI 필수 검사로 만드세요.",
      "Commit an AIGate pre-push hook file or rely on required server-side CI.": "AIGate pre-push hook 파일을 커밋하거나 서버측 필수 CI에 의존하세요.",
      "Automate hook activation in prepare/postinstall or rely on required server-side CI.": "prepare/postinstall에서 hook 활성화를 자동화하거나 서버측 필수 CI에 의존하세요.",
    ...
    ja: {
      ...
      "Install the pre-push hook or add aigate git-ready to CI required checks.": "pre-push hook をインストールするか、CI 必須チェックに aigate git-ready を追加してください。",
      "Make aigate git-ready a required CI check or install the local pre-push hook.": "aigate git-ready を CI 必須チェックにするか、ローカル pre-push hook をインストールしてください。",
      "Make aigate git-ready a verified required CI check.": "aigate git-ready を検証済みの CI 必須チェックにしてください。",
      "Commit an AIGate pre-push hook file or rely on required server-side CI.": "AIGate pre-push hook ファイルを commit するか、server-side の必須 CI に任せてください。",
      "Automate hook activation in prepare/postinstall or rely on required server-side CI.": "prepare/postinstall で hook 有効化を自動化するか、server-side の必須 CI に任せてください。",
    ...
    zh: {
      ...
      "Install the pre-push hook or add aigate git-ready to CI required checks.": "请安装 pre-push hook，或把 aigate git-ready 加入 CI 必需检查。",
      "Make aigate git-ready a required CI check or install the local pre-push hook.": "请将 aigate git-ready 设为 CI 必需检查，或安装本地 pre-push hook。",
      "Make aigate git-ready a verified required CI check.": "请将 aigate git-ready 设为已验证的 CI 必需检查。",
      "Commit an AIGate pre-push hook file or rely on required server-side CI.": "提交 AIGate pre-push hook 文件，或依赖服务器端必需 CI。",
      "Automate hook activation in prepare/postinstall or rely on required server-side CI.": "在 prepare/postinstall 中自动激活 hook，或依赖服务器端必需 CI。",
